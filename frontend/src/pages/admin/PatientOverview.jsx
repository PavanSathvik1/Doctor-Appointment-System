...
const PatientOverview = () => {
    ...
    useEffect(() => {
        const fetchOverview = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/patients/overview', {
                    ...
                    {overview?.latestPrescription ? (
                        <div style={{ flex: 1, padding: '1.5rem', backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                {overview.latestPrescription.medications.slice(0, 2).map((med, i) => (
                                ...
                                {overview.latestPrescription.medications.length > 2 && (
                                    <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--primary)', textAlign: 'center' }}>
                                        + {overview.latestPrescription.medications.length - 2} more medications
                                    </p>
                                )}
                            </div>
                        </div>
